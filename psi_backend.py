...
"""CPU and CUDA implementations of the PRS-CS GIG/psi update."""
...
class CpuPsiBackend:
    """Existing fused Numba implementation."""
...
class CudaPsiBackend:
    ...
    def sample(self, out, a_minus_half, delta, beta, sigma, n):
        ...
        if out.size != self._size:
            raise ValueError("out has %d values; expected %d" %
                             (out.size, self._size))
        if delta.size != self._size or beta.size != self._size:
            raise ValueError("delta, beta and out must have the same length")
        sigma = float(sigma)
        n = int(n)
        ...
        with self._device:
            ...
            s_large = cp.where(
                ...
                cp.sqrt(
                    4.0 /
                    cp.maximum(alpha * np.cosh(1.0) + lam, tiny)
                ),
            )
            safe_alpha = cp.maximum(alpha, tiny)
            s_alpha = cp.log(
                1.0 + 1.0 / safe_alpha +
                cp.sqrt(1.0 / (safe_alpha * safe_alpha) + 2.0 / safe_alpha)
            )
            if lam == 0.0:
            ...
            if bool(cp.any(active).item()):
                ...
                raise RuntimeError(
                    ...
                    "%d active draw(s)" % (self._max_rounds, remaining)
                )
...
class CudaRawPsiBackend:
    ...
    def __init__(self, size, seed=None, cuda_device=0,
        ...
        try:
            ...
            raise RuntimeError(
                "The CUDA raw psi backend requires CuPy. Install the "
    ...
    def sample(self, out, a_minus_half, delta, beta, sigma, n):
        ...
        if np.any(failed):
            raise RuntimeError(
                "CUDA raw GIG rejection sampler exceeded %d rounds with "
                "%d active draw(s)" %
                (self._max_rounds, int(np.count_nonzero(failed)))
            )
        if not np.isfinite(draws).all() or not np.all(draws > 0.0):
            raise RuntimeError(
                "CUDA raw GIG rejection sampler produced invalid draws"
            )
    ...
    def describe(self):
        return "cuda-raw:%d (single-kernel GIG; Philox 4x32-10)" % (
            self._device.id
        )

    def profile_summary(self):
        if not self._sample_calls:
            return "CUDA raw GIG: no draws recorded"
        return (
            "CUDA raw GIG: mean %.2f rejection rounds/vector, max %d" %
            (
                self._rounds_total / float(self._sample_calls),
                self._rounds_maximum,
            )
...
class CudaFusedPsiBackend(CudaRawPsiBackend):
    """Single-kernel CUDA update that generates delta and then psi."""
    ...
    def sample_joint(self, out, a_minus_half, gamma_shape, old_psi, phi,
        ...
        """Generate delta and psi together, returning delta's sum if needed."""
        cp = self._cp
        out = np.asarray(out)
        ...
        if old_psi.size != self._size or beta.size != self._size:
            raise ValueError("old_psi, beta and out must have the same length")
        if gamma_shape <= 0.0:
    ...
    def describe(self):
        return (
            "cuda-fused:%d (single-kernel delta + GIG; Philox 4x32-10)" %
            self._device.id
        )
...
def make_psi_backend(backend, size, seed=None, cuda_device=0,
    ...
    raise ValueError(
        "unknown psi backend %r; expected 'cpu', 'cuda', 'cuda-raw' or "
        "'cuda-fused'" % backend
    )
