...
`cuda-streams` preserves the same FP64 transition while assigning independent buckets to separate non-blocking CUDA streams, each with its own cuSOLVER handle, cuBLAS handle and factorization workspace. This is intended for real panels containing many large sparse buckets that do not individually saturate the GPU:
