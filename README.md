...
The default comparison runs `cpu`, `cuda`, and `cuda-pcg`. Use, for example, `--backends=cuda,cuda-pcg --n-iter=100` for a longer GPU-only comparison. Add `--psi-backend=cuda` to benchmark the vectorized CUDA GIG update, `--psi-backend=cuda-raw` for the single-kernel GIG implementation, or `--psi-backend=cuda-fused` to include `delta` generation in that kernel. The benchmark does not install or modify CUDA packages or drivers.
