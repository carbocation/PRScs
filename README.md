...
- LD_CACHE_DIR (optional): Directory for persistent filtered and PSD-projected LD caches. Cache entries are keyed by the source LD file metadata and the exact ordered SNP/allele-flip selection, so changing the reference, selected variants or allele orientation creates a cache miss rather than reusing incompatible matrices. The first matching run writes a potentially large uncompressed cache; later runs skip the original LD read, filtering and eigendecomposition. No cache is read or written by default.
