...
class CudaBetaBackend:
    ...
    def __init__(self, ld_blocks, block_sizes, beta_mrg, n_gwas,
        ...
        host_dtype = np.dtype(_kwargs.get("_cuda_dtype", np.float64))
        if host_dtype not in (np.dtype(np.float32), np.dtype(np.float64)):
            raise ValueError("CUDA beta dtype must be float32 or float64")
        self._host_dtype = host_dtype
        self._cuda_dtype = cp.dtype(host_dtype)
        ...
        with self._device:
            ...
            self._beta_result = cp.empty(
                self._p + 1, dtype=self._cuda_dtype
            )
            self._psi_device = cp.empty(self._p, dtype=self._cuda_dtype)
            ...
            for padded_size, blocks in sorted(buckets.items()):
                ...
                host_beta_mrg = np.zeros(
                    (count, padded_size, 1), dtype=self._host_dtype
                )
...
class CudaDirectBetaBackend(CudaBetaBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._one = np.array(1.0, dtype=self._host_dtype)
        if self._host_dtype == np.dtype(np.float32):
            self._potrf_batched = self._cusolver.spotrfBatched
            self._trsm_batched = self._cublas.strsmBatched
        else:
            self._potrf_batched = self._cusolver.dpotrfBatched
            self._trsm_batched = self._cublas.dtrsmBatched
...
class CudaHybridBetaBackend(CudaDirectBetaBackend):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self._host_dtype == np.dtype(np.float32):
            self._potrf_single = self._cusolver.spotrf
            self._potrf_buffer_size = self._cusolver.spotrf_bufferSize
            self._trsm_single = self._cublas.strsm
        else:
            ...
            if not self._potrf_checked:
                for group_index, group in enumerate(self._groups):
                    ...
                    if failures.size:
                        first = int(failures[0])
                        raise RuntimeError(
                            "hybrid CUDA Cholesky failed in size bucket "
                            "%d, matrix %d with info=%d" %
                            (group_index, first, int(info[first]))
                        )
    ...
    def describe(self):
        return (
            "cuda-hybrid:%d (exact FP64; regular potrf/trsm for %d "
            "matrices in %d sparse buckets, batched for %d matrices in "
            "%d buckets; %.1f MiB static resident)" %
            (
                self._device.id,
...
class CudaStreamsBetaBackend(CudaHybridBetaBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self._groups:
            raise ValueError(
                "cuda-streams requires at least one active LD block"
            )
            ...
            if not self._potrf_checked:
                for group_index, group in enumerate(self._groups):
                    ...
                    if failures.size:
                        first = int(failures[0])
                        raise RuntimeError(
                            "streamed CUDA Cholesky failed in size bucket "
                            "%d, matrix %d with info=%d" %
                            (group_index, first, int(info[first]))
                        )
...
class CudaFp32BetaBackend(CudaDirectBetaBackend):
    """Approximate FP32 direct Cholesky and triangular-solve backend."""
    ...
    def describe(self):
        return (
            "cuda-fp32:%d (preallocated FP32 potrfBatched + "
            "trsmBatched; approximate; %d size buckets; %.1f MiB "
            "static resident)" %
            (
                self._device.id,
                len(self._groups),
