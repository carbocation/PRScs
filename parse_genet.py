...
def parse_sumstats_chromosomes(ref_dicts, vld_dict, sst_file, n_subj):
    """Align joint-chromosome sumstats using two total file scans."""
...
def _ld_cache_key(chr_name, sst_dict, return_factors):
    ...
    digest.update(b'with-factors' if return_factors else b'blocks-only')
...
def _read_ld_cache(cache_file, cache_key, return_factors):
    try:
        with h5py.File(cache_file, 'r') as cached:
            if (int(cached.attrs['version']) != _LD_CACHE_VERSION or
                    cached.attrs['key'] != cache_key or
                    bool(cached.attrs['return_factors']) != return_factors):
                return None
            ...
            ld_factors = [
                np.array(blocks[str(index)]['factor'])
                for index in range(len(blk_size))
            ]
            ld_eigenvalues = [
                np.array(blocks[str(index)]['eigenvalues'])
                for index in range(len(blk_size))
            ]
...
def _write_ld_cache(cache_file, cache_key, ld_blk, blk_size,
                    ld_factors, ld_eigenvalues, return_factors):
    ...
    try:
        ...
        with h5py.File(temporary, 'w') as cached:
            ...
            cached.attrs['return_factors'] = return_factors
            ...
            for index, ld in enumerate(ld_blk):
                ...
                if return_factors:
                    block.create_dataset('factor', data=ld_factors[index])
                    block.create_dataset(
                        'eigenvalues', data=ld_eigenvalues[index]
                    )
...
def parse_ldblk(ldblk_dir, sst_dict, chrom, return_factors=False,
                report_timing=False, cache_dir=None):
    ...
    if cache_dir is not None:
        cache_key = _ld_cache_key(chr_name, sst_dict, return_factors)
        ...
        if os.path.isfile(cache_file):
            cached = _read_ld_cache(cache_file, cache_key, return_factors)
    ...
    for blk in range(n_blk):
        ...
        if idx != []:
            ...
            ld_blk[blk], factor, eigenvalues = _project_ld_psd(ld_blk[blk])
