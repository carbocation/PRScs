...
class JointChromosomeCliTests(unittest.TestCase):
    def test_joint_chromosomes_flag_uses_upstream_boolean_style(self):
        argv = [
            'PRScs.py',
...
class JointChromosomeInputTests(unittest.TestCase):
    def test_chromosome_inputs_are_concatenated_with_output_slices(self):
        ...
        self.assertEqual(combined['chromosome_slices'], [(1, 0, 2), (2, 2, 5)])
    ...
    def test_joint_mode_invokes_one_sampler_for_selected_chromosomes(self):
        ...
        with mock.patch.object(PRScs, 'parse_param', return_value=parameters):
            with mock.patch.object(
                ...
                with mock.patch.object(PRScs, '_print_ld_diagnostics'):
...
class JointChromosomeSamplerTests(unittest.TestCase):
    ...
    def test_one_chain_updates_global_parameters_and_splits_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            # sigma, the auxiliary global-scale draw, and phi are each
            # sampled once.  The phi shape uses all five SNPs rather than a
            # chromosome-specific variant count.
            ...
            self.assertTrue(all(line.startswith('1\t') for line in chromosome_one_lines))
            self.assertTrue(all(line.startswith('2\t') for line in chromosome_two_lines))
